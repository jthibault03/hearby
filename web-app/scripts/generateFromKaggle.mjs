...
async function loadExistingSongs(baseDir) {
  try {
    ...
    for (const key of Object.keys(data)) {
      ...
      const artistKey = (song.artistNames || [])
        .join(",")
        .toLowerCase()
        .trim();
...
async function main() {
  ...
  for (const row of records) {
    ...
    const title = row.track_name || row.track || row.name || null;
    ...
    // Try to merge year, albumArtUrl, and existing moodTags from previous JSON
    let year = null;
    const titleKey = (title || "").toLowerCase().trim();
    const artistKey = artistNames.join(",").toLowerCase().trim();
    const composite = `${titleKey}__${artistKey}`;
    ...
    if (existing) {
      ...
      if (existing.albumArtUrl) albumArtUrl = existing.albumArtUrl;
